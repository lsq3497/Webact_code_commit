...
class Agent:
    ...
    def call_model_with_message(self, system_prompt=None, messages=None):
        ...
        # 处理返回值：可能是字符串（旧版本）或元组（新版本，包含token使用）
...
class Actor(Agent):
    def __init__(self, config, objective, prompt_template, plan_tree_node):
        ...
        # 使用配置的日志目录，如果存在则使用，否则使用 CURRENT_DIR
    ...
    def try_repair_actspec_action(self, action_str):
        ...
        if re.match(r"^actspec\s*\[", s):
            return action_str
        # 匹配 actspec <action_id> 或 actspec <action_id>,param1="v1",...
        # action_id 仅允许字母数字、点、冒号、下划线、连字符
    ...
    def is_valid_action(self, action_str):
        ...
        match action:
            ...
            case "actspec":
                # 验证ActSpec action格式
                # 统一推荐格式：actspec [full_action_id],param1="value1",param2="value with spaces"
                match = re.search(r"^actspec\s*\[(?P<id>[^\]]+)\]", action_str.strip())
                if not match:
                    print(f"[调试] is_valid_action: actspec动作格式错误 - {repr(action_str)}")
                    return False
                # 如果env中有ActSpec库，验证action_id是否存在，且必须来自候选集中给出的完整ID
    ...
    def are_valid_actions(self, actions):
        ...
        # 对于其他动作，使用标准解析流程
        # 将actspec添加到动作列表中，以便parse_str_to_action_list能够识别
    ...
    def get_available_actspecs(self, env=None, context=None):
        ...
        if env and hasattr(env, 'actspec_library') and env.actspec_library:
            try:
                ...
                # 构建上下文查询（含最近动作历史，用于 ActSpec 的 action_history_prefix 匹配）
                # 这里只依赖 env._get_recent_action_history_types 的存在；若不存在则忽略历史。
                history_types: list = []
                ...
                # 查询匹配的ActSpec（传递library_path参数，确保使用正确的路径）
                ...
                # 检查pre-condition，过滤不满足pre-condition的ActSpec
                if actspecs:
                    try:
                        # 尝试获取page对象
                        page = None
                        ...
                        # 获取observation文本（用于pre-condition检查）
                        # 优先使用 actor 已持有的当前 observation，避免重复调用 env.observation() 导致日志重复
                        ...
                        if page or observation_text:
                            ...
                            for actspec in actspecs:
                                ...
                                if pre_condition:
                                    # 检查pre-condition是否满足
                                    try:
                                        # 传递observation_text给pre_checker
                                        ...
                                        # 如果pre-condition检查出错，记录警告但不过滤（保守策略）
                                        action_id = actspec.get("action_id", "unknown")
                                        print(f"[警告] ActSpec {action_id} pre-condition检查出错: {e}，保留该ActSpec")
                                
                                # pre-condition满足或没有pre-condition，添加到结果列表
                            ...
                            # 如果无法获取page对象，跳过pre-condition检查（保守策略）
                            print(f"[ActSpec] 无法获取page对象，跳过pre-condition检查")
                    except Exception as e:
                        # 如果pre-condition检查过程出错，记录警告但不过滤（保守策略）
                        print(f"[警告] Pre-condition检查过程出错: {e}，保留所有ActSpec")
                        import traceback
                        traceback.print_exc()
                
                # 如果存在负约束过滤器，过滤被禁止的ActSpec
    ...
    def get_actor_instruction(self, examples=None, env=None):
        ...
        # 添加ActSpec信息到prompt中
        # 如果 use_negative_constraints_only 为 true，不添加 ActSpec 到 prompt
        ...
        if not use_negative_constraints_only and env and hasattr(env, 'actspec_merger') and env.actspec_merger:
            try:
                ...
                if available_actspecs:
                    ...
                    # 额外打印一次 ActSpec 提示片段，便于在终端日志中直接查看本轮提供给 LLM 的 ActSpec 提示内容
                    try:
    ...
    def predict_action(self, criticism_elements):
        ...
        for _ in range(self.config.number):
            ...
            while not get_valid_actions:
                ...
                # 检查响应是否为空（call_model_with_message 已经会抛出异常，这里作为双重检查）
...
class AgentOccam:
    ...
    def init_actor(self, env=None):
        ...
        # 保存env引用和sites信息，以便Actor可以访问ActSpec库
        if env:
            self.actor.env = env
    ...
    def act(self, objective, env):
        ...
        while not env.done():
            ...
            # 打印action详情（与 env 统一使用 1-based 步数，避免同一步出现 Step 0 / Step 1 两种编号）
            print(f"\n{'#'*100}")
            ...
            # 检查是否有负约束拦截错误，如果有，将其添加到 observation summary
            ...
            if hasattr(env, 'last_negative_constraint_error') and env.last_negative_constraint_error:
                # 将负约束拦截错误信息添加到 observation summary
